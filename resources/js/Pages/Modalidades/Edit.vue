...
const page = usePage();
const modalidadFromPage = page.props.modalidad;

const form = useForm({
    nombre: modalidadFromPage?.nombre ?? '',
    precio_por_unidad: modalidadFromPage?.precio_por_unidad ?? '',
});

const submit = () => {
    form.put(route('modalidades.update', modalidadFromPage?.id));
};
</script>

<template>
    <Head title="Editar modalidad de liquidación" />
